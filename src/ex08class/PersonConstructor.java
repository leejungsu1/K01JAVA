...
/*
생성자(Constructor)
-클래스를 객체화(인스턴트화)할때 자동으로 호출되는 메소드
-자동으로 호출되며 개발자가 임의로 호출할 수 없다.
-반환값이 없다. 즉 특정 연산후 값을 반환할수 없다.
	따라서 void를 쓸 수 없다.
-그외 메소드가 가진 모든 특성을 가지고 있다(매개변수전달,
	오버로딩등)
...
생성자 내에서 this의 사용법
	...
	용도2 : 생성자내에서 멤버변수와 매개변수의 이름이 동일할때
		구분하기 위해 사용한다.
		this.변수=> 멤버변수
		변수=> 생성자의 매개변수
...
public class PersonConstructor {
	...
	//멤버메소드 : 멤버변수를 초기화 할 목적으로 정의한 메소드
	void intialize(String name, int age, String addr) {
		
		//해당 코드는 생성자내에서만 사용할수있다.
		//this(name, age, "미상");
		
		//멤버변수를 구분하기 위한 this는 사용할수 있다.
		this.name = name;
		this.age = age;
		this.addr = addr;
	}
