...
Fnction : 매개변수, 리턴값 둘다 있음
Predicate : 매개변수 있고, 리턴값은 boolean값으로 반환함
 */
	...
	*/
	...
	public static double avg(Predicate<Human> ph, List<Human> li) {
		...
		for(Human h : li) {//li 컬렉션의 크기만큼 반복
			/*
			첫변째 호출에서는 남자일때 true를 반환한다. 즉 남자의 점수가 카운트됨.
			두번째 호출은 여자.
			 */
			if(ph.test(h)==true) {
				pCount++;//인원수 증가
		...
		return (double)tCount / pCount;//평균값계산후 반환
	}///end of avg
