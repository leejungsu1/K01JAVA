...
-정적멤버변수 & 멤버메소드를 가질수있다.
-외부클래스의 인스턴스형 멤버는 사용할수없다.
-외부클래스명$내부클래스명.class파일이 생성된다.

※원래 클래스앞에는 static을 붙일수없으나 내부클래스에는 허용된다.
 */
class OuterClass{
	//인스턴스형 멤버변수
	...
	void outerMemberMethod() {
		/*
		외부클래스의 멤버메소드에서는 내부정적클래스의 멤버를
		접근할수있다.(Heap영역에서는 Method영역을 참조 할수있다.)
		 */
	...
	static void outerStaticMethod() {
		...
		/*
		외부클래스의 정적 멤버 메소드에서는 내부클래스의 정적멤버만
		접근할수있다.(Method영역에서는 Heap영역을 참조할수있다.)
		 */
...
public class E02InnerStaticClass {
	...
	public static void main(String[] args) {
		...
		/*
		외부클래스가 아닌 다른 클래스에서 접근할때의 방법은 아래와같다.
		 */
		
		/*
		내부정적클래스의 정적멤버에 접근할때는 외부클래스의 객체생성없이
		클래스명으로 바로 접근이 가능하다.
		 */
		...
		/*
		내부정적클래스의 인스턴스형 멤버에 접근할때는 객체를 생성한후
		참조변수를 통해서 접근해야한다.
		 */
