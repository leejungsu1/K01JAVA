...
내부 멤버 클래스
	...
	-외부클래스의 모든 멤버(정적 혹은 인스턴스형)를 사용할수있다.
	-정적멤버를 가질수없다.(단, 상수는 가능하다.)
	-컴파일시 외부클래스명$내부클래스명.class파일이 생성된다.
	-이벤트기반 프로그래밍(윈도우, 안드로이드등)에서
	주로 많이 사용된다.
 */
...
class OuterMember{
	...
	static int outerStatincVar;//정적멤버변수
...
public class E01InnerMemberClass {
	...
	public static void main(String[] args) {
		...
		/*
		외부클래스가 아닌 다른클래스에서는 내부멤버클래스가
		보이지 않으므로 직접 인스턴스를 생성할수없다.
		 */
		//InnerMember innerClass = new InnerMember();//[에러발생]
		
		/*
		외부클래스의 인스턴스를 먼저생성한후 내부클래스의
		인스턴스를 생성해야한다.
		 */
		OuterMember outerClass = new OuterMember();
		
		/*
		내부클래스의 객체는 아래 3가지 방법을 통해 생성할수있다.
		 */
