...
-이름이 없는 클래스를 말한다.
-내부클래스처럼GUI프로그램에서 많이 사용된다.
-부모클래스의 메소드를 오버라이딩 하는것이 목적이다.
-클래스를 정의하는 것이 마치 메소드를 정의하는것과 비슷하다.
...
public class E03AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		부모타입의 참조변수로 하위객체에서 확장한 멤버에 접근할수없으므로
		위부분은 에러가 발생한다.		
		 */
		
		//Student클래스를 상속잗아 정의한 익명클래스(즉, Student의 하위 익명객체)
		Student anonyStydent = new Student("김사랑", "2017") {
			//익명클래스에서 확장한 멤버변수
			int age = 1;
		...
		anonyStydent.printInfo();
		/*
		하위 익명클래스레서 확장한 변수이므로 익명객체를 통해서는
		접근불가.
		 */
		//anonyStydent.age = 100;//접근불가-에러발생
		//((Student)anonyStydent).age = 100;//다운캐스팅 불가-에러발생
		
		/*
		다운캐스팅은 "(자식클래스명)부모타입의 참조변수" 형태로
		형변환하여 하위클래스의 멤버에 접근하기 위한 수단이다.
		그러나 익명클래스는 클래스명이 없기 때문에 위와같이 다운캐스팅이
		불가능하다.
		즉, 익명클래스에서는 새롭게 정의한 멤버변수를 접근할수없다.
		따라서 익명클래스는 오버라이딩 목적으로만 사용해야한다
		 */
		/*
