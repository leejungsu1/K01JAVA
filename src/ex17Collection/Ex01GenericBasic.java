...
//오렌지 전용 박스 : Orange객체만 저장할수있는 클래스
class OrangeBox{
	Orange item;
...
//과일상자 : Object기반으로 모든객체를 저장할수있는 클래스
class FruitBox{
	Object item;
...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		/*
		상자1은 오렌지 전용박스이므로 다른 과일 (즉 객체)을 저장할수
		없다. 컴파일 에러가 발생하므로 실행자체가 불가능하다
		->자료형에는 안전하지만, 구현에는 불편함이 따른다.
		다른객체를 저장하기 위해서는 또다른 Box클래스를 생성해야
		한다.
		 */
		...
		/*
		Object를 기반으로 한 FruitBox는 구현에는 편리하나 코드레벨레서
		컴파일 에러가 발생하지 않으므로, 오류를 찾아내기가 훨씬 더 힘들다.
		따라서 자료형에 안전하지 못한 코드가 된다.
		 */
		FruitBox fBox = new FruitBox(new Orange(20));
		Orange orange3 = (Orange)fBox.pullOut();
		orange3.showInfo();
