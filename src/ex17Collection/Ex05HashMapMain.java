...
HashMap<K,V>
	...
	-키값은 중복이 될수없다. 중복될경우 덮어쓰기 처리된다.
	-키값으로 검색하므로 다른 컬ㄹ렉션보다 속도가 빠르다.
 */
public class Ex05HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		객체저장
			: 객체저장시 기본에 저장된 동일한 key값이 존재하면
			이전 객체가 반환된다. 만약 처음이라면 null값이
			반환된다.
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값:"+ map.put("name", "홍길동"));//->null 반환됨
		...
		5-2] 키값을 모를때 출력하기
			...
			Set계열의 컬렉션 형태로 가져온다.
			2.Map계열의 컬렉에는 처음부터 확장 for문을 사용하는
			것이 불가능하다.
