...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		List<E> 인터페이스 계열의 컬렉션
		 ...
		 아래와 같이 <T>부분을 생략하면 모든객체를 저장할수있는
		...
		list.add("트와이스");
		list.add(3,"워너원");
		//list.add(5,"소방차");//에러발생. 인덱스는 건너뛸수없다.
		list.add(list.size(),"오마이걸");//<-4번 인덱스에 입력됨
		list.add(list.size(),"방탄소년단");
		System.out.println("중복저장된 객체수:"+ list.size());
		 ...
		 */
		System.out.println(list.add("트와이스") ? "중복": "중복아님");
		...
		-컬렉션에 저장된 내용을 Iterator에게 알려주기 위해
			객체를 생성한다.
		-hasNext()메소드로 반환할때 객체가 있는지 검사하고,
			next()메소드로 해당객체를 반환한다.
