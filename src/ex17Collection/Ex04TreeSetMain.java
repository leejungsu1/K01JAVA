...
/*
TreeSet<T>컬렉션
-트리라는 구조를 기반으로 한 Set컬렉션이다.
-Set의 기본적인 특성은 동일하나, Tree알고리즘을 통해 정렬되어
저장된다. 정렬의 기준은 개발자가 직접 정의할수있다.
...
class MyString implements Comparable<MyString>{
	...
	가나다와 같은 사전순, 문자열의 길이등 다양하게 설정이 가능하다.
	아래코드는 오름차순(작은것->큰것)정렬일때 사용한다.
	1과-1의 위치가 바뀌면 내림차순이 된다.
	 */
	@Override
	public int compareTo(MyString paramStr) {
		//정렬의 순서는 문자열의 길이를 기분으로 한다.
		if(getLenth() > paramStr.getLenth()) 
			return 1;
		else if(getLenth() < paramStr.getLenth()) 
			return -1;
		else 
			return 0;
	}
