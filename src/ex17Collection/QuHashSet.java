...
/*
컬렉션 set에 저장된 객체를 이름으로 검색하자. 해당 정보가 있으면 해당 어벤져스의 정보를 출력하고 없으면
"해당 영웅은 없어요ㅜㅜ" 라는 메시지 출력해야 한다. 검색 부분은 Iterator를 통해 구현하도록 한다.
set계열의 컬렉션은 기본적으로 중복저장을 허용하지 않는다. 그러나 아래와 같이 새롭게 정의한 클래스에 대해서는
 equals(), hashCode() 메소드를 적절히 오버라이딩 처리해야 중복을 제거할 수 있다. 메소드 오버라이딩을 
...
//		result = prime * result + ((weapon == null) ? 0 : weapon.hashCode());
	...
	public boolean equals(Object obj) {
		//형변환
		Avengers comAvengers = (Avengers)obj;
		System.out.println("중복 제거용 equals()메소드 호출");
		if(comAvengers.name.equals(this.name) && comAvengers.heroName.equals(this.heroName)) {
			return true;
		}
		else {
			return false;
...
public class QuHashSet {
	...
	public static void main(String[] args) {
		...
		boolean searchFlag = false;//검색결과유무
		
		Iterator<Avengers> itr = set.iterator();
		while(itr.hasNext()) {
			Avengers avengers = itr.next();
			if(name.equals(avengers.name)) {
				//검색결과가 있다면 플레그를 변경
				searchFlag = true;
				//toString()메소드를 오버라이딩했으므로 객체를 즉시 출력가능
				System.out.println(avengers);
				//itr.next().toString();
			}
//			else {
//				System.out.println("해당 영웅이 없어요ㅜㅜ");
//			}
		}
		if(searchFlag==true) {
