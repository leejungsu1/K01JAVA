...
-멤버변수 앞에 static지정자가 붙으면 정적변수가 된다.
-정적변수는 JVM(자바가상머신)에 의해 프로그램이 로드되는
	시점에 초기화된다.
-main메소드가 실행되기 전에 이미 초기화 되므로 main메소드
	실행시 별도의 선언없이 변수를 사용할 수있다.
...
class MyStatic{
	...
	사용할수있다. 인스턴스형변수, 정적변수 둘다가능.
	 */
	void instanceMethod() {
		System.out.println("insatanceVar="+instanceVar);//접근가능
		System.out.println("staticVar="+staticVar);//접근가능
		staticMethod();//접근가능
	}
	
	/*
	[정적 멤버메소드] 정적메소드 안에서는 인스턴스멤버를
	사용할수없다. 정적멤버만 사용이 가능하다.
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		메소드영역에 하나만 생성되어 모든 객체가 공유하는 공유변수가 된다.
		따라서 아래 출력결과는 900, 900이된다.
		 */
		ms1.staticVar = 800;
		ms2.staticVar = 900;/*
			위 두개의 문장은 동일한 변수에 접근하고 있으나
			코드레벨에서는 마치 다른 변수처럼 보인다.
