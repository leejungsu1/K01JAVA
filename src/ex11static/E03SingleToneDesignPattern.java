...
싱글톤 디자인 패턴
	...
	2. 외부 클래스에서 생성자를 접근할수없으므로 new연산자를 
	통해 객체생성은 불가능해진다.
	3. 클래스내부에 정적메소드로 해당클래스의 객체를
	반환하도록 정의한다.
...
class SingleTone{
	...
	/*
	생성자를 private으로 선언하면 클래스 외부에서 접근할수
	없으므로 new연산자를 통해 객체생성은 불가능해진다.
	 */
	private SingleTone() {
		//기본생성자
	}
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		//생성자가 private으로 선언되었으므로 외부에서 접근 불가능
		//SingleTone st1 = new SingleTone();//[오류발생]
		
		
		/*
		생성자를 접근 할수없는 대신 getInstance()메소드를 통해서만
		객체의 참조변수를 반환받을수있다.
		 */
