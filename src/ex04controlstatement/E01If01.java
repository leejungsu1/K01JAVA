...
if문(조건문, 분기문)
	...
	-if문의 조건문은 반드시 비교식 혹은 논리식이여야 하낟.
	-즉 결과값이 boolean타입(true,false)이어야한다.
	-실행할 문장이 하나이인경우 중괄호 생략이 가능하다.
 */
...
//		}
		...
		/*
		if문의 조건은 아래와 같이 boolean값을 반환하는 조건식이어야한다.
		 */
		if(num%2==0) {
		...
		System.in.read()
				...
				이를 무시하기 위해 main()함수에서 throws처리를 해준다.
		 */
...
//		System.out.println("입력한문자:"+ (char)asciiCode);
		...
		※숫자의 구간을 정할때는 반드시 논리 AND를 사용해야한다.
		 */
		if(asciiCode>=48 && asciiCode<=57) {//둘다 같이 만족해야 될 경우(구간표시) &&(논리 AND)사용.
