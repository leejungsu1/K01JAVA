...
class BaseSpeaker extends Speaker{
	...
	/*
	 @Override : '어노테이션'이라 부르고 오버라이딩된
	 메소드에 붙여 컴파일러에게 알려주는 역할을 한다.
	 */
	@Override
	public void showState() {
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		부모클래스의 참조변수가 자식클래스의 객체를 참조할수있다.
		이경우 접근범위는 부모클래스로 제한한다.
		즉, 아래 호출부분에서 speaker참조변수를 통해서는 setBase()
		메소드는 호출할수없다. setBase()는 자식클래스에 정의된
		멤버메소드이기 때문이다. 
		...
		//speaker.setBase(40);//[에러발생] -> 초기화하지 못하여 0으로 출력
		speaker.showState();
	}
}
