...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		-byte, short형은 게임캐릭터의 움직임의 표현이나 음원정보등을 저장할때사용한다.
		 */
		...
		-큰자료형과 작은자료형을 연산하면 큰자료형으로 자동형변환된다.(데이터의 손실이 있기때문)
		-단 byte와 short는 예외적인 상황으로 생각하면 된다.(int형 연산을 좋아하는 CPU)
		 */
		long long1 = 1000, long2 = 2000;
		long result4 = int1 +long1;//int + long = long형
		...
		long long4 = 2200000000l; /*
			자바컴파일러(CPU)는 정수형상수를 기본적으로 int형으로 간주하기때문에 
			위 정수를 메모리에 올리는순간 에러가 발생한다. 
			이때는 접미사(L or l)을 붙여서 int형 자료가 아닌 long형 자료라는 것을 
			알려줘야 에러가 발생하지 않는다. 
